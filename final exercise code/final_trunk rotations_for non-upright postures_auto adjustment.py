...
# FIX: Added Leaky Integrator for Pelvis Yaw
# FIX 2: Added Neutral Dwell requirement to prevent false reps on return-to-center
# FIX 3: Added Pitch/Roll (Bending) angles to the status bar
# FIX 4: Auto-Tare and Manual Tare now also reset Pitch/Roll bias to cure drift
#remember to stay for at least 2s in the neutral position
...
# Coaching targets
AXIAL_TARGET_RIGHT_DEG  = 7.0   
AXIAL_TARGET_LEFT_DEG   = 7.0   
YAW_HYST_DEG            = 5.0   

MAX_YAW_SPEED_DPS       = 120.0 
MAX_COMP_ANGLE_DEG      = 20.0  # Increased from 15.0 to 20.0 to reduce false positives
MAX_PELVIS_DRIFT_DEG    = 15.0  
...
# NEW: Neutral Re-Arm Logic (Prevents swing-through false reps)
...
def main():
    ...
    # NEW: Rep Arming Logic
    reps_armed = True   # Start armed (assuming starting at neutral)
                        ...
                        try:
                            ...
                            if dt > (REL_MIN_DT_FRAC / SAMPLE_HZ):
                                ...
                                if PELVIS_LEAK_TAU_S > 0:
                                    leak_factor = np.exp(-dt / PELVIS_LEAK_TAU_S)
                            ...
                            if (yaw_rel_bias is not None) and (total_reps >= AUTO_TARE_MIN_REPS):
                                ...
                                if need_unlock or classic_drift:
                                    ...
                                    rel_axial_int = 0.0
                                    pelvis_yaw_int = 0.0 
                                    # FIX: Update bending biases (Pitch/Roll) to eliminate drift
                                    p_rel_bias = p_rel
                                    r_rel_bias = r_rel
                            ...
                            if msvcrt.kbhit():
                                ...
                                if key == 'z':
                                    ...
                                    # FIX: Update bending biases (Pitch/Roll) on manual tare
                                    p_rel_bias = p_rel
                                    r_rel_bias = r_rel
